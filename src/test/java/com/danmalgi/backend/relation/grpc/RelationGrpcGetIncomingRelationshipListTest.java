...
class RelationGrpcGetIncomingRelationshipListTest {
    ...
    @Mock
    private StreamObserver<RelationshipProto.GetIncomingRelationshipListResponse> responseObserver;
    ...
    void getIncomingRelationshipList_정상_요청이면_onNext와_onCompleted_호출() {
        ...
        when(relationService.getIncomingRequests(2L)).thenReturn(relations);

        Context ctx = Context.current().withValue(GrpcContext.USER_ID, 2L);
        ctx.run(() -> relationGrpc.getIncomingRelationshipList(Empty.getDefaultInstance(), responseObserver));
        ...
        verify(responseObserver).onNext(argThat(response ->
                response.getRelationshipsCount() == 1 &&
                        response.getRelationships(0).getUser().getProfileImageUrl()
                                .equals("https://signed.example.com/profiles/1/img?sig=xyz")
        ));
        verify(responseObserver).onCompleted();
    }

    @Test
    void getIncomingRelationshipList_결과가_없으면_빈_목록으로_응답() {
        when(relationService.getIncomingRequests(2L)).thenReturn(List.of());
